...
# Directory Setup
MODEL_SAVE_DIR = os.path.join(BASE_DIR, "models_comparison")
RESULT_SAVE_DIR = os.path.join(BASE_DIR, "results_comparison")
...
def calculate_metrics(hist):
    """
    Computes mIoU, Dice Score, and Recall from the confusion matrix.
    """
    epsilon = 1e-7
    tp = np.diag(hist)
...
def run_experiment(model_name, train_dl, val_dl, test_dl, num_classes):
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"   Val -> Train Loss: {train_loss/len(train_dl):.4f} | mIoU: {miou:.4f} | Dice: {mdice:.4f}")
    ...
    # Final Test (Load Best Model & Visualize)
    ...
    test_miou, test_dice, test_recall = calculate_metrics(hist)
    print(f"Final Result ({model_name}) -> mIoU: {test_miou:.4f}, Dice: {test_dice:.4f}")
...
def main():
    set_seed(SEED)
    print(f"Model Comparison Experiment Started on {DEVICE}")
    ...
    if results:
        ...
        save_path = os.path.join(RESULT_SAVE_DIR, "final_model_comparison_vis.csv")
